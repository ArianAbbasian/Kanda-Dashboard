...
async function loadTableUsers(page = 1) {
  try {
    ...
    if (!data.users || data.users.length === 0) {
      ...
      if (tbody)
        tbody.innerHTML =
          '<tr><td colspan="10" class="empty-state">هیچ کاربری یافت نشد</td></tr>';
...
function handleRowSelection(event) {
  ...
  // Sync map marker style immediately
...
function handleSelectAll(event) {
  ...
  if (isChecked) {
    ...
    const deselectedIds = Array.from(rowCheckboxes)
...
function findMarkerByUserId(userId) {
  ...
  vectorSource.forEachFeature((feature) => {
    if (
      feature.values_.user &&
      String(feature.values_.user.id) === String(userId)
    ) {
      foundMarker = feature;
    }
  });
